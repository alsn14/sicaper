...
use App\Http\Controllers\PengingatServiceController;
use App\Models\Pic;
...
Route::middleware(['auth', "localization"])-> group(function(){
    ...
    Route::controller(RoomController::class)->prefix("room")->group(function(){
        ...
        Route::post('/kode','detailByCode')->name('room.code');
        Route::get('/daftar-room','list')->name('room.list');

        Route::middleware(['employee.middleware'])->group(function(){
            Route::post('/simpan','save')->name('room.save');
            Route::post('/info','detail')->name('room.detail');
            Route::post('/ubah','update')->name('room.update');
            Route::delete('/hapus','delete')->name('room.delete');
        });
    });

    //Pic
    Route::controller(PicController::class)->prefix("pic")->group(function(){
        Route::get('/','index')->name('pic');
        Route::post('/kode','detailByCode')->name('pic.code');
        Route::get('/daftar-pic','list')->name('pic.list');

        Route::middleware(['employee.middleware'])->group(function(){
            Route::post('/simpan','save')->name('pic.save');
            Route::post('/info','detail')->name('pic.detail');
            Route::post('/ubah','update')->name('pic.update');
            Route::delete('/hapus','delete')->name('pic.delete');
        });
    });
    ...
    // daftar pengingat service
    Route::controller(PengingatServiceController::class)->prefix('pengingat-service')->group(function(){
        Route::get('/', 'index')->name('pengingat.index');
    });
    //Route::get('/regresi-pengingat-service', [PengingatServiceController::class, 'tampilkanPrediksi'])
    // ->name('service.regresi_pengingat_service');

    Route::get('/service/daftar-pengingat', [PengingatServiceController::class, 'daftarPengingat'])->name('service.pengingat');

    Route::post('/pengingat/{id}/sudah-diservice', [PengingatServiceController::class, 'tandaiSudahDiservice'])->name('pengingat.sudahDiservice');
    
    Route::post('/prediksi-service', [PengingatServiceController::class, 'prediksiService'])->name('prediksi.service');

    Route::get('/generate-ai-service', [PengingatServiceController::class, 'generateDariAI'])->name('pengingat.generate.ai');

    Route::get('/generate-prediksi-service', [PengingatServiceController::class, 'generatePrediksiService']);

    Route::get('/generate-prediksi-service', [PengingatServiceController::class, 'generatePrediksiService'])->name('pengingat.generate.ai');

    Route::get('/regresi-pengingat-service', [PengingatServiceController::class, 'tampilkanPrediksi'])->name('regresi.pengingat');


    // service record
    Route::prefix('service-records')->group(function () {
        Route::get('/', [ServiceRecordController::class, 'index'])->name('service-records.index');
        Route::get('/create/{itemId}', [ServiceRecordController::class, 'create'])->name('service-records.create');
        Route::post('/store/{itemId}', [ServiceRecordController::class, 'store'])->name('service-records.store');
        });
    
    // Rute untuk mengimport data ke database
    Route::get('/import-service-predictions', [PengingatServiceController::class, 'importServicePredictions'])->name('import.service.predictions');;


    // laporan service record
    Route::get('/export-service-records', [App\Http\Controllers\ExportController::class, 'exportServiceRecords'])->name('export.service_records');
